...
def get_child_businesses(parent_business_code):
    child_business = get_child_business(parent_business_code)
    for business in child_business:
        if business['is_leaf'] == 0:
            business['child'] = get_child_businesses(business['business_code'])
    return child_business


def get_child_business(parent_business_code):
    return pg_update.select("business", order_by='id', where=" parent_business_code = '%s' " % parent_business_code)
